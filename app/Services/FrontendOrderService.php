            ...
            DB::transaction(function () use ($request) {
                $this->frontendOrder = FrontendOrder::create(
                    $request->validated() + [
                        ...
                        'payment_status' => $request->is_payment_card === true ? 5 : 10,
                        'order_datetime'   => date('Y-m-d H:i:s'),
                        'preparation_time' => Settings::group('order_setup')->get('order_setup_food_preparation_time')
                    ]
                );
                if ($request->is_payment_card == true) {
                    if (!empty($request->uuid_transactions))
                    {
                        $transaction = Transactions::where('uuid',$request->uuid_transactions)->first();
                        if (!empty($transaction))
                        {
                            $transaction->fill([
                                'id_order'  =>  $this->frontendOrder->id
                            ]);
                            $transaction->save();
                        }
                    }
