...
export const typesVehicles = {
    ...
    actions: {
        ...
        save({ dispatch, commit, state }, payload) {
            return new Promise((resolve, reject) => {
                ...
                if (state.temp.isEditing) {
                    method = axios.post;
                    url = `/admin/type-vehicles/${state.temp.temp_id}`;
                }
        ...
        destroy({ dispatch }, payload) {
            return new Promise((resolve, reject) => {
                axios.delete(`/admin/type-vehicles/${payload.id}`)
                    .then(res => {
                        dispatch('lists', payload.search);
                        resolve(res);
                    })
        ...
        show({ commit }, id) {
            return new Promise((resolve, reject) => {
                axios.get(`/admin/type-vehicles/show/${id}`)
                    .then(res => {
        ...
        export(_, payload) {
            return new Promise((resolve, reject) => {
                let url = '/admin/type-vehicles/export';
                if (payload) {
                    url += appService.requestHandler(payload);
                }
